...
def hasil_prediksi():
    ...
    messagebox.showinfo("Hasil Prediksi", f"Prediksi Fakultas: {hasil_prodi}")
    ...
    cursor.execute("INSERT INTO nilai_siswa VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (nama, nilai_biologi, nilai_fisika, nilai_inggris,nilai_sosiologi, nilai_matematika,nilai_sejarah, nilai_tik, nilai_ppkn, nilai_geografi, hasil_prodi))
    conn.commit()
    conn.close()
